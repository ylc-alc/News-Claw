...
def load_seen_topics():
    """
    Load seen-topics tracker.

    Expected format (new):  { "fingerprint_string": "YYYY-MM-DD", ... }
    Old format (legacy):    { "stories": [...], "last_updated": "..." }

    If the file is in the old format or contains any non-string values,
    we reset to an empty dict so update_seen_topics never receives a list
    as a date value. This happens cleanly on the first run after migration.
    """
    try:
        ...
        if not isinstance(data, dict):
            return {}
        # Old format guard: if any value is not a plain date string, reset.
        if any(not isinstance(v, str) for v in data.values()):
            print("[process_feeds] seen_topics.json is in legacy format — resetting.")
            return {}
        return data
    except Exception:
        return {}
